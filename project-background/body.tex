...
Alda ships with a read-evaluate-print loop for playing the music described in Alda code. This arguably a more native support than ABC's use of third-party software to play an ABC tune out loud. ABC on the other hand has more straightforward support for printing sheet music as a PDF, as there are software programs explicitly for this purpose. To produce sheet music from Alda code, the code must be exported to MIDI\cite{Yarwood15_2}, which can in turn be used to generate sheet music using software like MuseScore\cite{MuseScore17}. 
